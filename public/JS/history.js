...
function renderSpendByItem(items) {
    ...
    container.innerHTML = `
        <table class="report-table">
            <thead>
                <tr>
                    <th>Item</th><th>Qty Consumed</th><th>Qty Added</th><th>Net Change</th>
                    <th>Consumption Spend</th><th>Addition Spend</th><th>Avg Cost/Unit</th><th>Events</th>
                </tr>
            </thead>
            <tbody>
                ${items.map(item => {
    ...
    container.innerHTML = `
        ...
        <div class="top-consumed-list">
            ${items.map(item => {
                ...
                return `<div class="top-consumed-item">
                    <div class="top-consumed-info">
                        <span class="top-consumed-name">${escapeHtml(item.itemName)}</span>
                        <span class="top-consumed-stats">${escapeHtml(item.totalConsumed)} units &middot; ${formatCurrency(item.totalSpend)} &middot; ${escapeHtml(item.eventCount)} events</span>
                    </div>
...
function renderActivityByType(types) {
    ...
    const typeLabels = {
        'quantity_change': 'Quantity Change', 'quantity_consumed': 'Quantity Consumed',
        'item_used': 'Item Used', 'order_placed': 'Order Placed',
        'cycle_count': 'Cycle Count', 'item_created': 'Item Created',
        'item_updated': 'Item Updated', 'item_deleted': 'Item Deleted'
    };
    const total = types.reduce((s, t) => s + t.count, 0);
    container.innerHTML = `
        <table class="report-table">
            <thead><tr><th>Change Type</th><th>Count</th><th>% of Total</th><th>Distribution</th></tr></thead>
            <tbody>
                ${types.map(t => {
                    ...
                    return `<tr>
                        ...
                        <td>${pct.toFixed(1)}%</td>
                        <td><div class="activity-bar-bg"><div class="activity-bar" style="width: ${pct}%"></div></div></td>
                    </tr>`;
                }).join('')}
            </tbody>
        </table>
...
function renderUserActivity(users) {
    ...
    container.innerHTML = `
        <table class="report-table">
            <thead><tr><th>User</th><th>Actions</th><th>Qty Consumed</th><th>Qty Added</th></tr></thead>
            <tbody>
                ${users.map(u => `<tr>
...
function renderDailyTrend(days) {
    ...
    container.innerHTML = `
        <table class="report-table">
            <thead><tr><th>Date</th><th>Events</th><th>Consumed</th><th>Added</th><th>Daily Spend</th><th>Activity</th></tr></thead>
            <tbody>
                ${days.map(d => {
                    ...
                    return `<tr>
                        ...
                        <td><div class="activity-bar-bg"><div class="activity-bar trend-bar" style="width: ${pct}%"></div></div></td>
                    </tr>`;
                }).join('')}
            </tbody>
            <tfoot>
